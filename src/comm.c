...
/*
	...
	write_to_buffer( d, "	(N)one!	      - look for a class later\n\r\n\r", 0);
        write_to_buffer( d, "What is your class  (W/V/D/R/M/J/N)? ", 0 );
...
//        ch->level = 0;
    ...
    case CON_READ_MOTD:
	write_to_buffer( d,     "\n\rWelcome to Godwars: The Lords of War.\n\rMay thy blade stay ever sharp, thy soul ever dark.\n\r",
	    0 );
	ch->next	= char_list;
	char_list	= ch;
	d->connected	= CON_PLAYING;
...
*/
/*
	if (!str_cmp(ch->name,"Antilles") && IS_CLASS( ch,CLASS_DEMON) &&
...
if (IS_IMMORTAL(ch) && !IS_SET(chars_logged_in, LOGIN_GOD)) 
	...
	sprintf(buf,"#g%s has entered#b The Lords of War.#n", ch->name);
	do_info(ch,buf);
	if(ch->version < VERSION){
	  stc("Player File updated.",ch);ch->version = VERSION;}
