...
!IS_CLASS(ch, CLASS_VAMPIRE) && !IS_SET(ch->special, SPC_CHAMPION) )
    ...
    else if (IS_CLASS(ch, CLASS_VAMPIRE) && ch->pcdata->powers[VAM_PROT] < 1 && ch->pcdata->powers[VAM_OBTE] < 3) 
    {
...
void do_shadowsight( CHAR_DATA *ch, char *argument )
{
	...
	if ((IS_CLASS(ch, CLASS_VAMPIRE) || IS_ABOMINATION(ch)) && ch->pcdata->powers[VAM_OBTE] < 2)
	{
...
*/
        ...
        if (!str_cmp(ch->name,"Sage")) sprintf(buf,"Sage the Coder");    
...
NULL, TO_ROOM );
        ...
        SET_BIT(ch->polyaff, POLY_ZULO);
        SET_BIT(ch->affected_by, AFF_POLYMORPH);
        if (!str_cmp(ch->name,"Infidel")) sprintf(buf,"Infidel the Coder");    
...
void do_clanname( CHAR_DATA *ch, char *argument )
{
    ...
    if ( IS_NPC(victim) ) return;
/*
    if (victim->pcdata->powers[MPOWER_RUNE0] == 0);
        {
                victim->clan = "Purple Archmage";
        return;
}

    if (victim->pcdata->powers[MPOWER_RUNE0] == 1);
        {
...
ch->pcdata->powers[VAM_ANIM],ch->pcdata->powers[VAM_CELE],
ch->pcdata->powers[VAM_FORT]);
send_to_char(buf,ch);
sprintf(buf,
"Obtenebration:[%d]             Presence:   [%d]        Quietus:     [%d]\n\r",
ch->pcdata->powers[VAM_OBTE],ch->pcdata->powers[VAM_PRES],
ch->pcdata->powers[VAM_QUIE]);
send_to_char(buf,ch);
sprintf(buf,
"Thaumaturgy:  [%d]             Auspex:     [%d]        Dominate:    [%d]\n\r",
ch->pcdata->powers[VAM_THAU],ch->pcdata->powers[VAM_AUSP],
ch->pcdata->powers[VAM_DOMI]);
send_to_char(buf,ch);
sprintf(buf,
"Obfuscate:    [%d]             Potence:    [%d]        Protean:     [%d]\n\r",
ch->pcdata->powers[VAM_OBFU],ch->pcdata->powers[VAM_POTE],
ch->pcdata->powers[VAM_PROT]);
send_to_char(buf,ch);
sprintf(buf,
"Serpentis:    [%d]             Vicissitude:[%d]        Daimoinon:   [%d]\n\r",
ch->pcdata->powers[VAM_SERP],ch->pcdata->powers[VAM_VICI],
ch->pcdata->powers[VAM_DAIM]);
send_to_char(buf,ch);
sprintf(buf,			 
"Chimerstry:   [%d]             Obeah:      [%d]        Melpominee:  [%d]\n\r",
ch->pcdata->powers[VAM_CHIM], ch->pcdata->powers[VAM_OBEA],
ch->pcdata->powers[VAM_MELP]);
send_to_char(buf,ch);
sprintf(buf,
...
void do_shadowplane( CHAR_DATA *ch, char *argument )
{
    ...
    else if (IS_CLASS(ch, CLASS_VAMPIRE) && ch->pcdata->powers[VAM_OBTE] < 3)
    {
        send_to_char("You are not trained in the Obtenebration discipline.\n\r",ch);
        return;
    }
...
void do_truesight( CHAR_DATA *ch, char *argument )
{
    ...
    if (( IS_CLASS(ch, CLASS_VAMPIRE) || IS_ABOMINATION(ch)) && ch->pcdata->powers[VAM_AUSP] < 1) 
    {
...
void do_scry( CHAR_DATA *ch, char *argument )
{
  ...
  if ( IS_CLASS(ch, CLASS_VAMPIRE) && ch->pcdata->powers[VAM_AUSP] < 2 )
    {
	...
	return;
	    ...
	    && !IS_CLASS(ch, CLASS_MONK) && !IS_ABOMINATION(ch))
    {
	send_to_char( "Huh?\n\r", ch );
	return;
    }
...
void do_shield( CHAR_DATA *ch, char *argument )
{
	...
	&& !IS_CLASS(ch, CLASS_MAGE) && !IS_ABOMINATION(ch))
    {
	stc("Huh?\n\r",ch);
	return;
    }
   ...
   if (IS_CLASS(ch, CLASS_DROW) && !IS_SET(ch->pcdata->powers[1],DPOWER_DROWSHIELD))
    ...
    if (IS_CLASS(ch, CLASS_VAMPIRE) &&  ch->pcdata->powers[VAM_OBFU] < 2)
    {
...
/*old ww regen :P removed to add agg damage sage 1/1/00
...
void vamp_rage( CHAR_DATA *ch )
{
    ...
    if (!IS_VAMPAFF(ch, VAM_NIGHTSIGHT) && ch->pcdata->powers[VAM_PROT] > 0)
        do_nightsight(ch,"");
    if (!IS_VAMPAFF(ch, VAM_FANGS)) do_fangs(ch,"");  
    if (!IS_VAMPAFF(ch, VAM_CLAWS) && ch->pcdata->powers[VAM_PROT] > 1)
        do_claws(ch,"");
...
void do_upkeep( CHAR_DATA *ch, char *argument )
{
    ...
    {mageupkeep(ch);return;}
    if(IS_CLASS(ch,CLASS_WEREWOLF))
    {wwupkeep(ch);return;}
    if(IS_CLASS(ch,CLASS_DROW))
    {drowupkeep(ch);return;}
    if(IS_CLASS(ch,CLASS_MONK))
    {monkupkeep(ch);return;}
...
if (ch->pcdata->powers[VAM_PROT] > 8) send_to_char("Your flesh is as hard as marble.\n\r",ch);
...
void do_vclan( CHAR_DATA *ch, char *argument )
{
	...
	{
	    sprintf( buf,
	    "[%-16s] [  %d  ] [%-6ld%3ld] [%-6d%3d] [%-6d%3d] [%7d] [ %3d ]\n\r",
		capitalize( gch->name ),
...
void do_tribe( CHAR_DATA *ch, char *argument )
{
	...
	{
	    sprintf( buf,
	    "[%-16s] [%-13s] [%-6ld%3ld] [%-6d%3d] [%-6d%3d] [%7d]\n\r",
...
void do_werewolf( CHAR_DATA *ch, char *argument )
{
    ...
    SET_BIT(ch->affected_by, AFF_POLYMORPH);
    SET_BIT(ch->pcdata->stats[UNI_AFF], VAM_DISGUISED);
      if (IS_ABOMINATION(ch) && IS_POLYAFF(ch, POLY_ZULO))
...
void do_vanish( CHAR_DATA *ch, char *argument )
{
    ...
    if (( IS_CLASS(ch, CLASS_VAMPIRE) || !IS_ABOMINATION(ch)) && ch->pcdata->powers[VAM_OBFU] < 1)
    {
...
void do_klaive( CHAR_DATA *ch, char *argmuent )
{
...
//    char arg[MAX_INPUT_LENGTH];
 ...
 if ( ch->gifts[GALLIARD] < 2 )
    ...
    {
	send_to_char("Missing object, please inform SaGe.\n\r",ch);
	return;
    }
    ch->practice   -= 85;
